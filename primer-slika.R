...
for(r in 1:nrow(imgvec)) { #pogledam, ali je kateri pixel dovolj blizu med sabo - nato poračunam, kako se razlikujeta po barvi
  simetricna[r,]=ifelse(abs(imgvec[r,1]-imgvec[,1])<=epsilon & abs(imgvec[r,2]-imgvec[,2])<=epsilon,exp(-(imgvec[r,3]-imgvec[,3])^2/sigma),0)
}
#naredim spectral clustering na nenormalizirani laplaceovi matriki
